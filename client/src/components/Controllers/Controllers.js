...
export function validateDuracion(input) {
	...
	if (input.duracion === 'min' && input.min !== '') {
		if (input.min > 60) {
			error.min =
				"Si la duracion es mayor de 60 minutos, elija la opcion de 'Horas'";
		} else if (input.min < 0) {
		...
		if (input.hs !== '') {
			if (input.hs > 24) {
				...
				error.hs =
					"Si la duracion es menor de una hora, elija la opcion 'Minutos'";
...
export function validateField(obj) {
	...
	if (obj.nombre === '' || obj.temporada === '' || obj.dificultad === '') {
		error.field = 'No pueden haber campos vacios';
	}
	if (/[^A-Za-z ]/g.test(obj.nombre)) {
		error.nombre = 'Solo letras en el nombre';
